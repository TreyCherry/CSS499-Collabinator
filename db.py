...
def add_alert(db, user_id, message): # Add a new alert for a user
    db.execute('INSERT INTO alerts (user_id, message) VALUES (?, ?)',
               (user_id, message))
    db.commit()

def get_alerts(db, user_id): # Retrieve all alerts for a user
    return db.execute('SELECT * FROM alerts WHERE user_id = ?', (user_id,)).fetchall()
